...
class BoardArray {
    ...
    hasGameOver() {
        return JSON.stringify(this.board) === this.boardSnapshot && !this.hasPossibleMove();
    }

    getRandomValues(): number {
        const randomValue = this.possibleValues[getRandomIndex(this.possibleValuesLength)];
        ...
        for (let i = 1; i < BOARD_HEIGHT; i++) {
            for (let j = 0; j < BOARD_WIDTH; j++) {
                ...
                while (row > 0) {
                    ...
                    debug(`currentCell id ${currentCell.id} currentCell value ${currentCell.val}`);
